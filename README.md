...
Version 4.x introduces the unified dual-engine architecture, merging the local backend engine and AI Studio client engine into one seamless workspace. Version 4.4.4 restores sequential multi-image batch generation with live progressive stage previews (aligning with the standalone Lite experience) so completed slots preview immediately without waiting for the full batch to finish, while resolving Direct Mode large workspace thumbnail disappearance caused by snapshot compaction through savedFilename fallbacks, synchronous memory cache resolution, and high-fidelity IndexedDB snapshot hydration. Version 4.4.3 fixes a Direct Mode runtime ReferenceError by restoring the missing loadImageDimensions import in browserDirectProvider, ensuring accurate actual output dimensions and session hints measurement without spurious generation failure logs across all image models. Version 4.4.2 delivers AI Studio Direct Mode transient 401 token refresh resilience and subscription quota protection, automatically handling background OAuth 2.0 / session cookie latency on cold starts with a bounded 1.5s retry to guarantee flawless generation without wasting slots. Version 4.4.1 resolves Image Editor Shared Settings resolution synchronization and guards model constraints so active user resolution settings (e.g. 2K or 4K) are preserved without being overridden by loaded base image dimensions. Version 4.4.0 solves large workspace persistence and image/thought disappearance in AI Studio (Direct) mode, integrating IndexedDB v2 dual-snapshot backing to break through the 5MB browser quota ceiling (supporting up to 1GB workspaces), three-tier memory caching for outputs, thoughts, and thumbnails, guaranteed atomic database writes, and direct mode virtual path alignment. Version 4.3.0 delivered comprehensive Google AI Pro (1x 200k TPM) and Ultra (5x / 20x) subscription pacing restructure, dynamic adaptive workload delay calculation for 4K resolution, multi-reference images, and High Thinking mode, tier-aware 429 safety margin buffers (3.5s for Pro, 2.0s for Ultra), anti-resonance backoff breaking 60s sliding window locks, and an offline workspace snapshot circuit breaker. It also integrates the Gemini 3.8 Flash (`gemini-3.8-flash`) prompt engineering upgrade and interactive 429 Cooldown Modal with live seconds countdown and user action control. Version 4.2.0 introduced the initial multi-tier Google AI Subscription Plan selector with persistent backoff and global IPM rate protection.
