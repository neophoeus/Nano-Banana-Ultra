...
4.x 版本導入雙引擎統一架構，將本機後端引擎與 AI Studio 客戶端引擎無縫整合於同一套工作區。4.4.4 版本還原批次圖像生成的循序處理循環與主舞台漸進式即時預覽（對齊獨立 Lite 版體驗），單槽完成即時在主舞台呈現成品並解除載入遮罩，無須等待整批完成；同時修復 Direct 模式在大容量工作區觸發快照壓縮後因 URL 清空導致縮圖誤判為缺損佔位框的問題，透過 savedFilename 回退、記憶體快取即時同步解析與高精度 IndexedDB 快照水合，確保縮圖顯示零丟失與零閃爍。4.4.3 版本修復 Direct 模式中因 browserDirectProvider 遺漏 loadImageDimensions 模組匯入而引發的執行階段 ReferenceError 異常，確保所有圖片模型在產圖後均能準確量測實際產出解析度與寫入 session hints，避免成功生成的圖片被錯誤判定為失敗並拋出錯誤日誌。4.4.2 版本為 AI Studio Direct 模式建立瞬時 401 換證容錯與訂閱額度保護機制，自動處理冷啟動時背景 OAuth 2.0 / Session Cookie 換證延遲，透過上限 1 次的 1.5 秒有界自動重試確保生成順暢不浪費卡槽，並優化認證異常引導提示。4.4.1 版本修復圖像編輯器懸浮「共享設定」中的解析度同步問題，移除底圖像素強制覆蓋解析度的舊行為，並為無尺寸約束模型加上動態能力檢查防護，確保使用者在首頁設定的 2K/4K 等解析度在進入編輯器時能被完整保留與同步。4.4.0 版本徹底解決 AI Studio（Direct）模式下大容量工作區的圖片與思考過程丟失問題，導入 IndexedDB v2 雙重快照備份以突破瀏覽器 5MB 配額上限（支援至 1GB 大型工作區）、三層獨立記憶體快取（主圖 25、思考圖 50、縮圖 80）、保障 IndexedDB 異步落盤，並修正 Direct 模式虛擬路徑對齊。4.3.0 版本針對 Google AI Pro（1x 基準配額，200k TPM）與 Ultra（5x / 20x）訂閱全面重構調用步調，導入針對 4K 解析度、參考圖與 High Thinking 思考圖的動態負載自適應間隔調速，將 429 退避安全裕度提升至 3.5 秒（Pro）/ 2 秒（Ultra），並打破 60 秒滑動窗口諧波共振鎖死與增加離線工作區快照斷路器。同時整合了 Gemini 3.8 Flash（`gemini-3.8-flash`）提示詞工程升級與 429 限流冷卻即時倒數互動彈窗。4.2.0 版本引進了 Google AI 訂閱體系（Pro / Ultra 5x / Ultra 20x）專屬節奏切換與基礎退避重試能力。
