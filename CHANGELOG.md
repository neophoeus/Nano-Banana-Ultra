...
- Release title: Nano Banana Ultra 4.5.0 - Full Lite Feature Integration Milestone & Zero-Regression Quota Protection
- Release summary:
    - **Definitive Unification & Integration Milestone**: Marks the complete and definitive integration of all capabilities, safety mechanisms, error handling, and reliability protections from the standalone `App-Nano_Banana_Ultra_lite` edition into `Nano Banana Ultra`, while guaranteeing zero regression for Ultra's advanced multi-tier subscription and adaptive quota management architecture.
    - **Thinking Loop Circuit Breaker (`services/providers/browserDirectProvider.ts`)**: Ported `detectThinkingLoop` to detect repetitive reasoning patterns in model thought outputs via sliding-window phrase frequency analysis (triggering on >= 5 identical clauses of length >= 12) along with an absolute 12,000-character safety limit. Aborts runaway thinking loops with a non-retryable error, preventing runaway token consumption and hang states.
    - **Persistent Cross-Reload Rate Limit Backoff (`services/providers/browserDirectProvider.ts`)**: Persisted model rate limit backoff deadlines into `localStorage` (`nbu_model_rate_limit_backoff_until_${model}`). Prevents browser reloads from bypassing active 429 cooling periods while incorporating self-healing logic to automatically prune corrupted or excessively distant timestamps (> 120s).
    - **Startup Memory Preloading & Workspace Reset Cleanup (`utils/workspacePersistence.ts`, `index.tsx`, `App.tsx`, `utils/workspaceExecutionMode.ts`)**: Introduced `preloadWorkspaceImagesToMemory(snapshot)` to hydrate workspace thumbnails and active stage images into global synchronous memory before UI mount, eliminating thumbnail flicker and latency in large direct-mode workspaces. Introduced `clearStoredWorkspaceSnapshot()` to cleanly purge both localStorage and sessionStorage on workspace reset while calling `clearBrowserSavedImageRecords()` to wipe orphaned IndexedDB blobs. Fixed `setStoredExecutionModeSetting` to immediately mark detection as completed.
    - **Dedicated Quota Exceeded Status & 9-Language Localization (`types.ts`, `utils/generationFailure.ts`, `utils/translations/`)**: Added `'quota-exceeded'` as a first-class `GenerationFailureCode` with top-priority specificity. Automatically classifies 429 and `RESOURCE_EXHAUSTED` responses and renders localized error copy across all 9 supported languages (`zh_TW`, `zh_CN`, `en`, `ja`, `ko`, `de`, `es`, `fr`, `ru`).
    - **Auto-Backup Toast Display Duration Adjustment (`hooks/usePerformGeneration.ts`)**: Extended the auto-backup success notification duration to 8000ms (from 4000ms), giving users ample visibility when snapshots are saved.
    - **Ported & Expanded Unit Test Suites (`tests/geminiCredentials.test.ts`, `tests/geminiRequestConfig.test.ts`, `tests/imageSaveUtils.browserPersistence.test.ts`, `tests/browserDirectProvider.test.ts`, `tests/generationFailure.test.ts`, `tests/workspacePersistence.test.ts`)**: Ported missing Lite test suites into Ultra and expanded existing suites to verify thinking loop detection, rate limit persistence, quota-exceeded classification, and memory lifecycle helpers.
    - **Zero-Regression Quota Protection Guarantee**: 100% preservation of Ultra's Google AI Studio subscription tier system (`pro` 1x, `ultra_5x` 5x, `ultra_20x` 20x), dynamic workload-based pacing calculations (4K, 2K, multi-references, High Thinking), tier safety margins (+4s/+2s/+1s), interactive 429 Cooldown Modal, and Gemini 3.8 Flash model support. Total test suites expanded to 119 files and 992 tests, passing at 100%.
